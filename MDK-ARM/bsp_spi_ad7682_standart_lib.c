...
//		GPIO_InitStruct.Pull      = GPIO_PULLUP;  //下拉一个
...
void MX_TIM8_Init(void)
{
  ...
  htim8.Instance = TIM8;
  htim8.Init.Prescaler = 0;//1830 0 32768
  htim8.Init.CounterMode = TIM_COUNTERMODE_UP;
  htim8.Init.Period = 1830;//3660;//1830;//1831=120000000/65526的采样率;
...
//    Error_Handler();
	...
	__HAL_SPI_ENABLE_IT(&SpiHandle, SPI_IT_RXNE);  // 使能串口空闲中断
...
//  0011 11XX X111 0001  0x3A71 
#define SAMPLE_CFG  0x3A71

volatile uint16_t ad7682_rec[20];
uint16_t ad7682_cfg=0;
const uint16_t ad7682_ch_queue[12]={1,2,1,3,1,0,1,2}; //原始
//const uint16_t ad7682_ch_queue[12]={1,1,1,1,1,1,1,1};
...
//		HAL_SPI_IRQHandler(&SpiHandle);
	 uint16_t spidata=0;
	 if(RESET != __HAL_SPI_GET_FLAG(&SpiHandle, SPI_FLAG_RXNE))   
...
//            printf("\r\nUART1 Idle IQR Detected\r\n");
  	...
  	spidata=	SPI1->DR;
		ad7682_date[cur_ad_index++]=spidata;//[cur_ad_ch];//test++;//
		if(spidata<30000)
		{
//			bsp_LedToggle(1);
			test++;
		}
//		ad7682_date[cur_ad_index++]=test;//ad7682_rec[cur_ad_ch];
		cur_ad_ch++;
...
//			 osSemaphoreRelease(ad7682_readyHandle);
		...
		else if(cur_ad_index>=(SAMPLE_ADCH*SAMPLEPOINTS*2))//-SAMPLE_ADCH
		{
			cur_ad_index=0;			
